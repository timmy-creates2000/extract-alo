...
// Which ALOC batch endpoint to poll:
//   'q' (DEFAULT) → /q/{N}  — returns up to ~10-20 random questions/call. The endpoint that survived
//                             our key's throttle the LONGEST; use this first.
//   'm'           → /m      — returns ~40 random questions/call (more efficient) but was throttled first.
//                             Only worth trying if a fresh IP (e.g. Replit) reset the key's quota.
...
async function fetchBatch(subject, year, attempt = 0) {
  const url =
    ENDPOINT === 'm'
      ? `${ALOC_BASE}/m?subject=${encodeURIComponent(subject)}&year=${year}&type=utme`
      : `${ALOC_BASE}/q/${BATCH_N}?subject=${encodeURIComponent(subject)}&year=${year}&type=utme`
  try {
...
async function main() {
  console.log('📥 ALOC → Supabase extraction' + (GAPS_ONLY ? '  [GAPS_ONLY]' : '') + `  [endpoint=/${ENDPOINT}]`)
